...
st.set_page_config(page_title="Regular Staff | CH Bathinda", page_icon="🏥", layout="wide")
...
def enforce_anti_leak_ui():
    ...
    <style>
        ...
        .enterprise-table th { background-color: #1d4ed8; color: white; padding: 16px; text-align: left; }
...
def initialize_session():
    if "regular_verified" not in st.session_state:
        st.session_state.regular_verified = False

def authenticate(user, pwd):
    try:
        if user == st.secrets["ADMIN_USERNAME"] and pwd == st.secrets["ADMIN_PASSWORD"]:
            st.session_state.regular_verified = True
            return True
        return False
    except Exception:
...
# ==========================================
# 3. CLOUD DATABASE CONTROLLER (WORKSHEET 3)
# ==========================================
@st.cache_data(ttl=300)
def fetch_regular_data():
    ...
    try:
        ...
        # 🎯 STRICT TARGET: 3rd Worksheet (Index 2)
        # 0 = 1st, 1 = 2nd, 2 = 3rd Sheet
        worksheet = spreadsheet.get_worksheet(2) 
        
        if worksheet is None:
            st.error("⚠️ Error: 3rd Worksheet nahi mili. Check karein ki Google Sheet mein 3 tabs hain.")
            return pd.DataFrame()
        ...
        # Blank duplicate headers ignore mechanism
...
def render_auth_gateway():
    ...
    with col2:
        st.markdown("<h2 style='text-align: center; color: #1d4ed8;'>🔐 Regular Staff Portal</h2>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center;'>Authorized Login Required</p>", unsafe_allow_html=True)
        
        with st.form("regular_security_form"):
...
def render_regular_dashboard():
    col_header, col_action = st.columns([4, 1])
    with col_header:
        st.title("🏥 Regular Staff Roster")
    ...
    with st.spinner("Decrypting Regular Staff Data Matrix..."):
        df = fetch_regular_data()

    if df.empty:
        st.warning("⚠️ Roster is empty or connection failed.")
        return

    st.markdown("#### 🔍 Search & Filter Staff Records")
    
    # 🎯 SMART SEARCH BAR
    search_term = st.text_input("🔎 Search by Employee Name, Designation, or Posting Location:", placeholder="Type to search...")
    ...
    # --- Smart Search Logic ---
    if search_term:
        # Yeh line saare columns ko string banakar search term match karegi (Case-insensitive)
    ...
    if not processed_df.empty:
        ...
        st.info("⚠️ Koi record nahi mila. Kripya apna search term check karein.")
...
if not st.session_state.regular_verified:
    render_auth_gateway()
else:
    render_regular_dashboard()
